...
const EN_MESSAGES = {
  ...
  modelProtocolCapabilitiesDescription: "Configure how PI uses protocol features supported by each gateway model. Keep PI defaults unless the gateway requires a correction.",
  ...
  usePiDefaultCurrent: "Use PI default · currently {value}",
  enabled: "Enabled",
  disabled: "Disabled",
  resetAllOverrides: "Restore all PI defaults",
  ...
  resetIncompatibleOverride: "Restore the incompatible protocol capability {option} for {model} to its PI default before changing the protocol",
...
const ZH_CN_MESSAGES: Record<ManagedProviderMessageKey, string> = {
  ...
  modelProtocolCapabilitiesDescription: "配置 PI 如何使用网关模型支持的协议特性。除非网关需要修正，否则保持 PI 默认值。",
  ...
  capabilityColumn: "能力",
  policyColumn: "设置方式",
  effectiveColumn: "当前有效值",
  piDefaults: "PI 默认",
  customSettingsCount: "已自定义 {count} 项",
  forceEnabled: "强制开启",
  forceDisabled: "强制关闭",
  usePiDefaultCurrent: "使用 PI 默认值 · 当前{value}",
  enabled: "开启",
  disabled: "关闭",
  resetAllOverrides: "全部恢复为 PI 默认值",
  ...
  resetIncompatibleOverride: "修改模型协议前，请先将 {model} 的不兼容协议能力 {option} 恢复为 PI 默认值",
